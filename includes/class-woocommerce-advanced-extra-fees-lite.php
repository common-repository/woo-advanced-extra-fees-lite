...
class Woocommerce_Advanced_Extra_Fees_Lite {
	...
	private function define_admin_hooks() {
		...
		if ( 'plugins.php' == $pagenow ) :
			// Plugins page
			$this->loader->add_filter( 'plugin_action_links_' . WAEF_LITE_PLUGIN_BASENAME , $plugin_admin,  'waef_lite_add_plugin_action_links', 10, 2 );
		endif;
	...
	// Enqueue WooCommerce Style And Script
	public function set_wc_screen_ids($screen){
		$screen[] = 'toplevel_page_waef'; 
		return $screen; 
	}
	/**
	...
	public static function clean_notices() {
		if(isset($_GET['page']) && !empty($_GET['page']) && $_GET['page'] == 'waef'){
			...
			add_action( 'admin_notices', array( __CLASS__, 'admin_notices' ), 999 );
		}
	}	
